...
void readDimention() {
  ...
  if (valx >= 0 && valx <= 179 && valy >= 0 && valy <= 179) {
    ...
    if (valx < 27) {
      ...
      readGPS();
      bot.sendMessage(id,"Person falling.");
      if(message == ""){
        bot.sendMessage(id,"No GPS data available.");
      }else{
        bot.sendMessage(id,message);
  ...
  for (int i = 0; i < numNewMessages; i++) {
    ...
    if (text == "/sendlocation") {
      ...
      if (lat_str != "" && lng_str != "") {  // Only send if location is updated
        ...
        bot.sendMessage(chat_id, "Location: " + message);  // Send updated GPS location
      } else {
        bot.sendMessage(chat_id, "GPS location not available yet.");
      }
    }
  }
}
