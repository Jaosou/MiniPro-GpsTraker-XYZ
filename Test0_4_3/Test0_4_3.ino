...
//Connect Telegram
const char* token = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx";
...
void readDimention() {
  ...
  if (valx >= 0 && valx <= 179 && valy >= 0 && valy <= 179) {
    ...
    if (valx < 26) {
      ...
      readGPS();
      bot.sendMessage(id_chat,"\n--------------------------------------------------\nคนกำลังล้ม.");
      if(message == ""){
        bot.sendMessage(id_chat,"ไม่มีสัญญา GPS.\n--------------------------------------------------\n");
      }else{
  ...
  for (int i = 0; i < numNewMessages; i++) {
    ...
    if (text == "/sendlocation") {
      ...
      if (lat_str != "" && lng_str != "") {  // Only send if location is updated
        ...
        bot.sendMessage(chat_id, "\n--------------------------------------------------\nโลเคชั่นปัจจุบัน : " + message+"\n--------------------------------------------------\n");  // Send updated GPS location
      } else {
        bot.sendMessage(chat_id, "\n--------------------------------------------------\nไม่มีสัญญา GPS \n--------------------------------------------------\n");
      }
    }else if(text == "/start" && countStart == 0){
      id_chat = bot.messages[i].chat_id;
      Serial.println(id_chat);
      countStart = 1;
      bot.sendMessage(id_chat,"เชื่อมต่อแล้ว : \nพร้อมใช้งาน");
    }
  }
}
